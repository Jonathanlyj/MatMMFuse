...
def get_dataset(data_path, target_index, reprocess="False", processing_args=None, model_name="GraphJepa"):
...
class StructureDataset_large(Dataset):
    def __init__(
        ...
        self.data_path = data_path
        self.processed_path = processed_path
        super(StructureDataset_large, self).__init__(data_path, transform, pre_transform)
...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        if processing_args["data_format"] != "db":
            file_path =os.path.join(data_path, structure_id + "." + processing_args["data_format"])
            if os.path.exists(file_path) == False:
                print(f"Structure file {index} not found, skipping	...")
